...
| `Task tool (superpowers:task-reviewer)` | `@generalist` with the filled `task-reviewer-prompt.md` template (one reviewer, spec + quality) |
| `Task tool (superpowers:code-reviewer)` | `@generalist` with the filled `requesting-code-review/code-reviewer.md` template |
